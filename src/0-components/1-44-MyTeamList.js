...
export default ({ item }) => (
  <div className="plr30 bg-white">
    ...
    <List item={item.goods} href="/0-home/1-product-detail" as={`/product/${item.goods_id}`} />
    <div className="h110 flex jc-between ai-center">
      <HideImg imgList={item.fans} imgSize={50} margin={-0.1} teamId={item.launch_id} />
      <div>
        <WrapLink
          path={`/my/details/${item.id}`}
          className="w150 h50 r10 font24 ml20 c999 border-default flex-inline jc-center ai-center"
        >
          拼单详情
        </WrapLink>
        <WrapLink
          path={`/my/order/${item.order_id}`}
